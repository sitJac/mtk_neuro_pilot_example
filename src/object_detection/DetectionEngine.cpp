...
const static int kOutputTensorDetectBoxesIdx = 0;
const static int kOutputTensorDetectClassesIdx = 1;
const static int kOutputTensorDetectScoresIdx = 2;
const static int kOutputTensorDetectNumDetectionsIdx = 3;
...
DetectionEngine::DetectionEngine(const std::string& model_path, const std::string& label_path,
    ...
    RETURN_IF_NP_ERROR(
        ANeuralNetworksTFLiteOptions_setAccelerationMode(mOptions, NP_ACCELERATION_NEURON));
    RETURN_IF_NP_ERROR(ANeuralNetworksTFLiteOptions_setCacheDir(mOptions, "/data/local/tmp/"));
    // Default execution preference is Fast Single Answer. If user has power concern, user can
    // choose Sustained Mode.
    RETURN_IF_NP_ERROR(
...
void DetectionEngine::InitBuffers() {
    ...
    RETURN_IF_NP_ERROR(ANeuroPilotTFLiteWrapper_getTensorByteSize(
        mTFLite, TFLITE_BUFFER_TYPE_OUTPUT, kOutputTensorDetectBoxesIdx, &mOutputBoxesByteSize));
    ...
    RETURN_IF_NP_ERROR(ANeuroPilotTFLiteWrapper_getTensorByteSize(
        mTFLite, TFLITE_BUFFER_TYPE_OUTPUT, kOutputTensorDetectClassesIdx,
        &mOutputClassesByteSize));
    ...
    RETURN_IF_NP_ERROR(ANeuroPilotTFLiteWrapper_getTensorByteSize(
        mTFLite, TFLITE_BUFFER_TYPE_OUTPUT, kOutputTensorDetectScoresIdx, &mOutputScoresByteSize));
    ...
    RETURN_IF_NP_ERROR(ANeuroPilotTFLiteWrapper_getTensorByteSize(
        mTFLite, TFLITE_BUFFER_TYPE_OUTPUT, kOutputTensorDetectNumDetectionsIdx,
...
bool DetectionEngine::Inference(const void* buffer, const std::size_t buffer_size,
    ...
    // Copy the output tensor data to allocated buffer
    {
        ...
        RETURN_FALSE_IF_NP_ERROR(ANeuroPilotTFLiteWrapper_getOutputTensorData(
            mTFLite, kOutputTensorDetectBoxesIdx, mOutputBoxesBuffer, mOutputBoxesByteSize));
    }

    {
        PROFILE_ME_AS("Copy output classes");
        RETURN_FALSE_IF_NP_ERROR(ANeuroPilotTFLiteWrapper_getOutputTensorData(
            mTFLite, kOutputTensorDetectClassesIdx, mOutputClassesBuffer, mOutputClassesByteSize));
    }

    {
        PROFILE_ME_AS("Copy output scores");
        RETURN_FALSE_IF_NP_ERROR(ANeuroPilotTFLiteWrapper_getOutputTensorData(
            mTFLite, kOutputTensorDetectScoresIdx, mOutputScoresBuffer, mOutputScoresByteSize));
    }

    {
        PROFILE_ME_AS("Copy output detections");
        RETURN_FALSE_IF_NP_ERROR(ANeuroPilotTFLiteWrapper_getOutputTensorData(
            mTFLite, kOutputTensorDetectNumDetectionsIdx, mOutputDetectionsBuffer,
            mOutputDetectionsByteSize));
    }
