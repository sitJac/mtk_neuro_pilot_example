...
bool run_inference(Settings* s) {
    ...
    std::vector<uint8_t> byte_buffer = utils::ReadBinaryFile(s->input_bin_path);
    std::size_t input_file_size = byte_buffer.size();
    if (input_file_size == 0) {
        LOG_ERROR(TAG, "Failed read input file: %s", s->input_bin_path.c_str());
        return false;
    }

    detection::ModelDataType model_type = engine.GetModelInputDataType();
    const void* input_ptr = reinterpret_cast<const void*>(byte_buffer.data());
    std::size_t input_size = input_file_size;

    // Tranform byte data to float value
    if (model_type == detection::ModelDataType::TYPE_FLOAT_32) {
        float_buffer = std::vector<float>(input_file_size);
        for (auto i = 0; i < input_file_size; i++) {
            float_buffer[i] = (byte_buffer[i] - s->input_mean) / s->input_std;
        }
        input_ptr = reinterpret_cast<const void*>(float_buffer.data());
        input_size = input_file_size * sizeof(float);
    } else if (model_type == detection::ModelDataType::TYPE_UINT8) {
        input_ptr = reinterpret_cast<const void*>(byte_buffer.data());
    ...
    bool ret = engine.Inference(input_ptr, input_size, results);
    if (ret) {
        // Print the results
