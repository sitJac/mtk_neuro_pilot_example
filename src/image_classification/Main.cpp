...
#define FLOAT_MODEL_PATH "/data/local/tmp/mobilenet_v1_224.tflite"
#define QUANT_MODEL_PATH "/data/local/tmp/mobilenet_v1_224_quant.tflite"
#define INPUT_BIN_PATH "/data/local/tmp/grace_hopper.bmp_224224.bin"
#define LABEL_PATH "/data/local/tmp/labels_imagenet_slim.txt"

struct Settings {
    bool profiling = false;
    bool allow_fp16 = false;
    int loop_count = 1;
    float input_mean = PREDICT_INPUT_MEAN;
    float input_std = PREDICT_INPUT_STD;
    std::string model_path = FLOAT_MODEL_PATH;
    std::string input_bin_path = INPUT_BIN_PATH;
    std::string labels_path = LABEL_PATH;
    int number_of_results = PREDICT_NUMBER_OF_RESULTS;
    bool verbose = false;
};
...
bool run_inference(Settings* s) {
    ...
    classification::ClassificationEngine engine(s->model_path.c_str(), s->labels_path.c_str(),
                                                s->allow_fp16);

    engine.SetVerboseLog(s->verbose);
    engine.SetNumberOfResults(s->number_of_results);
    ...
    do {
        ...
        classification::ModelDataType model_type = engine.GetModelInputDataType();

        // Tranform byte data to float value
        if (model_type == classification::ModelDataType::TYPE_FLOAT_32) {
        ...
        } else if (model_type == classification::ModelDataType::TYPE_UINT8) {
        ...
        LOG_INFO(TAG, "Inference with classification engine");
        // Inference with predict engine
        ret = engine.Inference(input_ptr, input_size, results);

        if (ret) {
            // Print the top-N results
            LOG_INFO(TAG, "Top-%d results", engine.GetNumberOfResults());
...
Settings process_options(int argc, char** argv) {
    ...
    while (1) {
        ...
        int option_index = 0;
        c = getopt_long(argc, argv, "b:c:f:i:l:m:p:r:s:v:", long_options, &option_index);
