# NeuroPilot Native Examples

## Introduction
Here are three examples using NeuroPilot APIs to do on-device inference.
1. You can find the Shim API at
   - `.\src\neuropilot_api\NeuroPilotTFLiteShim.h`

## How to build the cmake project
### Prerequisite
1. ANDROID_NDK_HOME env variable. Tested on 21.4.7075529.
2. `cmake` and `make` for building c++ executable
   - You may need MinGW for windows build
   - You can also use Ninja for both Linux/Windows build 

### Build Steps
1. cd and run the build script to build and push the executables
``` 
PS C:\path\to\NativeExample> .\00.build.bat
```
2. Play with the three built executables in the `build` folder and example scripts:
   1. For `GenericClassifier`: 01.run_basic_example.bat
   2. For `ClassifyImage`: 02.run_classify_example.bat
   3. For `DetectObject`: 03.run_detection_example.bat
